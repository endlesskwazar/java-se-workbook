...
Давайте спробуємо написати і скомпілювати, вже класичну, першу программу на Java, яка виводить на термінал повідомлення “Hello World”.
...
Інструмент javac читає визначення класів та інтерфейсів, написаних на мові програмування Java, і збирає їх у файли класу байт-кодів. Він також може обробляти анотації у вихідних файлах та класах Java.
...
В утиліти java є ряд стандартних опцій, які підтримуються на поточному середовищі виконання і будуть підтримуватися в майбутніх випусках. Розглянемо найбільш часто використовувувані параметри.
...
JAR-файл - це Java-архів (Java Archive). Це просто архівний файл, сжатий (іноді з нульовою компресією) по алгоритму zip.

JAR – файл може містити:
- файл маніфеста META-INF/MANIFEST.MF
...
- EAR (Enterprise Application aRchive) - містить у собі ентерпрайз програму (звичайно з декількох модулів).
...
- 'f' вказує, що ім’я вихідного файла буде jarfile (замість стандартного виводу за замовчуванням).
...
Дана команда запакує всі .class файли в робочій директорії і створить маніфест за замовчуванням.

![](../resources/img/1/3.png)

Для запуска jar – файла в утиліті java потрібно вказати параметр -jar з ім’ям файлу. Якщо ми спробуємо запустити щойно створений файл виникне помилка, оскільки в маніфесті відсутнє значення головного класу.

![](../resources/img/1/4.png)

Розпакуйте файл hello.jar і скопіюйте файл META-INF/MANIFEST.MF до корневої директорії. Змініть його вміст
```
...
3. Як працює JVM?
4. Поясніть процес зборки java - додатка.